...
class Question():
    ...
    def getView(self, window):
        ...
        Button(view, text=self.answers[0], command=lambda *args: self.check("A", view),width=40,font=("Times",10,"bold"),bg="lemon chiffon",fg="saddle brown").pack()
        Button(view, text=self.answers[1], command=lambda *args: self.check("B", view),width=40,font=("Times",10,"bold"),bg="lemon chiffon",fg="saddle brown").pack()
        Button(view, text=self.answers[2], command=lambda *args: self.check("C", view),width=40,font=("Times",10,"bold"),bg="lemon chiffon",fg="saddle brown").pack()
        Button(view, text=self.answers[3], command=lambda *args: self.check("D", view),width=40,font=("Times",10,"bold"),bg="lemon chiffon",fg="saddle brown").pack()
        return view
...
logo1 = PhotoImage(file="average.png")
w2 = Label(window, image=logo1,width=700,height=200).pack(side="top")

logo = PhotoImage(file="average1.png")
w1 = Label(window, image=logo,width=150,height=200).pack(side="right")
logo2 = PhotoImage(file="average1.png")
w3 = Label(window, image=logo2,width=150,height=200).pack(side="left")


label= Label(window, text="Welcome To Online Python Quiz",justify="left",width="150",height="2",font=("Lucida Handwriting",20),fg="red",bg="white")
label.pack()
label= Label(window, text="Instructions",width="50",height="2",font=("Lucida Handwriting",20),fg="gold",bg="white")
label.pack()
print("\n")
print("\n")
print("\n")
Label(window,text="1.Questions are related to python programming \n 2.Each Question as Four Options Out of which only one is correct \n 3.Correct Option will be displayed soon after answering \n 4.You cannot reback the question \n 5.final result will be displayed after the test completion",width="50",justify="left",font=("Berlin Sans FB",10),fg="black",bg="white").pack()
